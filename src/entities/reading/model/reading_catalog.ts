...
export const readingCatalog: readonly ReadingContent[] = [
  {
    id: "detailed-saju",
    code: "detailed-saju",
    title: "상세 사주",
...
export function getReadingAccessLabel(
  reading: Pick<ReadingContent, "availability" | "pricing">,
) {
  const { availability, pricing } = reading;
  ...
  if (pricing.type === "free") {
    return availability === "active" ? "무료" : "무료 예정";
  }

  if (pricing.type === "paid") {
    const priceLabel = formatKoreanWon(pricing.amount);

    return availability === "active" ? priceLabel : `${priceLabel} 예정`;
  }

  if (pricing.expectedAmount !== undefined) {
    return `${formatKoreanWon(pricing.expectedAmount)}대 예정`;
  }

  return "가격 검토 중";
}

export function getActiveReadingPrice(
  reading: Pick<ReadingContent, "availability" | "pricing">,
) {
  if (reading.availability !== "active" || reading.pricing.type !== "paid") {
    return undefined;
  }
