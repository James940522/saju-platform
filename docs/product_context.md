...
서비스명은 `선녀 사주`다. 사용자에게 보이는 서비스명은 이 표기를 일관되게 사용하고, 현재 repository 이름인 `saju-platform`은 개발상 식별자로만 취급한다.
...
- 사용자 노출 브랜드명은 `선녀 사주`로 통일하되, 코드와 architecture는 기능, 도메인, 사용자 행동 중심의 이름을 사용한다.
